...
static void glRasterLine() {
    ...
    if (rll < GlClip[0][Y] || rll > GlClip[1][Y])
        return;
    if (RasterRenderContext->ColorBuffer == NULL)
        return;
                ...
                */
                {
                    ...
                    */
                    ...
                    r = (fr1 * fracu * fracv) + (fr2 * (1 - fracu) * fracv) +
                        (fr3 * fracu * (1 - fracv)) +
                        (fr4 * (1 - fracu) * (1 - fracv));
                    g = (fg1 * fracu * fracv) + (fg2 * (1 - fracu) * fracv) +
                        (fg3 * fracu * (1 - fracv)) +
                        (fg4 * (1 - fracu) * (1 - fracv));
                    b = (fb1 * fracu * fracv) + (fb2 * (1 - fracu) * fracv) +
                        (fb3 * fracu * (1 - fracv)) +
                        (fb4 * (1 - fracu) * (1 - fracv));
                    ...
                    /*
                              switch (GlRenderFunc.ColorCombineFunction)
                              {
                                ...
                                case GR_COLORCOMBINE_TEXTURE_TIMES_CCRGB:
                                  ...
                                  g=DIV256(g*((GlRenderFunc.ConstantColor&0x0000FF00)>>
                       8));
                                  b=DIV256(b*((GlRenderFunc.ConstantColor&0x000000FF)>>
                       0)); break; case GR_COLORCOMBINE_TEXTURE_TIMES_ITRGB:
                       ...
                       GR_COLORCOMBINE_TEXTURE_TIMES_ITRGB_ADD_ALPHA:
                                  r=DIV256(r*FIXDN(rlv.r, ACR))+FIXDN(rlv.a,
                       ACA); g=DIV256(g*FIXDN(rlv.g, ACG))+FIXDN(rlv.a, ACA);
                                  b=DIV256(b*FIXDN(rlv.b, ACB))+FIXDN(rlv.a,
                       ACA); break; case GR_COLORCOMBINE_TEXTURE_TIMES_ALPHA:
                       ...
                       GR_COLORCOMBINE_TEXTURE_TIMES_ALPHA_ADD_ITRGB:
                                  r=DIV256(r*FIXDN(rlv.a, ACA))+FIXDN(rlv.r,
                       ACR); g=DIV256(g*FIXDN(rlv.a, ACA))+FIXDN(rlv.g, ACG);
                                  b=DIV256(b*FIXDN(rlv.a, ACA))+FIXDN(rlv.b,
                       ACB); break; case GR_COLORCOMBINE_TEXTURE_ADD_ITRGB:
                                  r=r+FIXDN(rlv.r, ACR);
                                  g=g+FIXDN(rlv.g, ACG);
                                  b=b+FIXDN(rlv.b, ACB);
                                  break;
...
EXPORT GLboolean APIENTRY glSetupSingleClipPlane(LPGLCLIPPLANE plane,
                                                 GLdouble *pos,
                                                 GLdouble *norm) {
...
EXPORT GLboolean APIENTRY glClipPolygonToSinglePlane(LPGLCLIPPLANE plane,
                                                     GLI32 *numin,
                                                     LPGLVERTEX vin,
                                                     GLI32 *numout,
                                                     LPGLVERTEX vout) {
    ...
    for (c = 0; c < (*numin); c++) {
        ...
        if (curin != nextin) {
            ...
            outvert->Screen[X] =
                invert->Screen[X] +
                ((vin[nextvert].Screen[X] - invert->Screen[X]) * scale);
            outvert->Screen[Y] =
                invert->Screen[Y] +
                ((vin[nextvert].Screen[Y] - invert->Screen[Y]) * scale);
            outvert->Screen[Z] =
                invert->Screen[Z] +
                ((vin[nextvert].Screen[Z] - invert->Screen[Z]) * scale);
            outvert->Screen[W] =
                invert->Screen[W] +
                ((vin[nextvert].Screen[W] - invert->Screen[W]) * scale);

            outvert->Color[R] =
                invert->Color[R] +
                ((vin[nextvert].Color[R] - invert->Color[R]) * scale);
            outvert->Color[G] =
                invert->Color[G] +
                ((vin[nextvert].Color[G] - invert->Color[G]) * scale);
            outvert->Color[B] =
                invert->Color[B] +
                ((vin[nextvert].Color[B] - invert->Color[B]) * scale);
            outvert->Color[A] =
                invert->Color[A] +
                ((vin[nextvert].Color[A] - invert->Color[A]) * scale);

            outvert->Tex[X] = invert->Tex[X] +
                              ((vin[nextvert].Tex[X] - invert->Tex[X]) * scale);
            outvert->Tex[Y] = invert->Tex[Y] +
                              ((vin[nextvert].Tex[Y] - invert->Tex[Y]) * scale);
...
EXPORT GLboolean APIENTRY glClipPolygon(LPGLCLIPPLANE ClipPlane,
                                        GLI32 NumClipPlane, GLI32 *numvin,
                                        LPGLVERTEX vin, GLI32 *numvout,
                                        LPGLVERTEX vout) {
    LPGLVERTEX curlist;
    ...
    for (c = 0; c < NumClipPlane - 1; c++) {
        if (glClipPolygonToSinglePlane(&(ClipPlane[c]), &curnumvin, curlist,
                                       numvout,
                                       &(TempVertices[curtemp][0])) == FALSE)
            return FALSE;
    ...
    return glClipPolygonToSinglePlane(&(ClipPlane[NumClipPlane - 1]),
                                      &curnumvin, curlist, numvout, vout);
}
...
void glPolygonToClipping(LPGLPOLYGON Polygon, GLdouble *Matrix) {
    ...
    for (c = 0; c < Polygon->NumVertex; c++) {
        glMatrix4dTransVector4d(Polygon->Vertex[c].Screen,
                                Polygon->Vertex[c].Eye, Matrix);
    }
}
...
void glLightPolygon(LPGLPOLYGON Polygon, LPGLRENDERBLOCK Block) {
    ...
    for (c = 0; c < Polygon->NumVertex; c++) {
        ...
        for (d = 0; d < GL_MAX_USER_LIGHTS; d++) {
            if (Block->Light[d].On) {
                glVector3dSub(Vertex2Light, Block->Light[d].Position,
                              Polygon->Vertex[c].Eye);
                ...
                Diffuse =
                    glVector3dDot(Vertex2Light, Polygon->Vertex[c].Normal);
                if (Diffuse < 0.0)
                    Diffuse = 0.0;
                ...
                Color[R] +=
                    Polygon->FrontFace.Ambient[R] * Block->Light[d].Ambient[R];
                Color[G] +=
                    Polygon->FrontFace.Ambient[G] * Block->Light[d].Ambient[G];
                Color[B] +=
                    Polygon->FrontFace.Ambient[B] * Block->Light[d].Ambient[B];

                // Add diffuse contribution
                Color[R] += Polygon->FrontFace.Diffuse[R] *
                            Block->Light[d].Diffuse[R] * Diffuse;
                Color[G] += Polygon->FrontFace.Diffuse[G] *
                            Block->Light[d].Diffuse[G] * Diffuse;
                Color[B] += Polygon->FrontFace.Diffuse[B] *
                            Block->Light[d].Diffuse[B] * Diffuse;

                // Add specular contribution
                Color[R] += Polygon->FrontFace.Specular[R] *
                            Block->Light[d].Specular[R] * Specular;
                Color[G] += Polygon->FrontFace.Specular[G] *
                            Block->Light[d].Specular[G] * Specular;
                Color[B] += Polygon->FrontFace.Specular[B] *
                            Block->Light[d].Specular[B] * Specular;
...
GLboolean glRasterSetContextAndBlock(LPGLRENDERCONTEXT Context,
    ...
    GlClip[0][X] = 0;
    GlClip[1][X] = Context->Width - 1;
    GlClip[0][Y] = 0;
    GlClip[1][Y] = Context->Height - 1;
...
void glRasterizePolygon(LPGLRENDERCONTEXT Context, LPGLRENDERBLOCK Block,
                        LPGLPOLYGON Polygon) {
    ...
    if (glCreatePolygon(&(ClippedPolygon), GL_MAX_POLY_VERTEX) == FALSE)
        return;

    glRasterSetContextAndBlock(Context, Block);

    glLightPolygon(Polygon, Block);
...
GLboolean glRasterSetContext(LPGLRENDERCONTEXT Context) {
    ...
    GlClip[0][X] = 0;
    GlClip[1][X] = RasterRenderContext->Width - 1;
    GlClip[0][Y] = 0;
    GlClip[1][Y] = RasterRenderContext->Height - 1;
