...
void glVector3fAdd
(GLfloat v1[3], GLfloat v2[3], GLfloat v3[3])
{
  v1[X]=v2[X]+v3[X];
  v1[Y]=v2[Y]+v3[Y];
  v1[Z]=v2[Z]+v3[Z];
...
void glVector3dAdd
(GLdouble v1[3], GLdouble v2[3], GLdouble v3[3])
{
  v1[X]=v2[X]+v3[X];
  v1[Y]=v2[Y]+v3[Y];
  v1[Z]=v2[Z]+v3[Z];
...
{
  if (k!=0.0) { vec[X]/=k; vec[Y]/=k; vec[Z]/=k; }
}
...
void glVector3fCross
(GLfloat res[3], GLfloat v1[3], GLfloat v2[3])
{
...
void glVector3dCross
(GLdouble res[3], GLdouble v1[3], GLdouble v2[3])
{
  res[X]=v1[Y]*v2[Z]-v1[Z]*v2[Y];
  res[Y]=v1[Z]*v2[X]-v1[X]*v2[Z];
  res[Z]=v1[X]*v2[Y]-v1[Y]*v2[X];
...
void glVector3fNormalize (GLfloat vec[3])
{
  ...
  if (len!=0.0) glVector3fInverseScale(vec, len);
  else { vec[X]=0; vec[Y]=0; vec[Z]=0; }
}
...
void glVector3dNormalize (GLdouble vec[3])
{
  ...
  if (len!=0.0) glVector3dInverseScale(vec, len);
  else { vec[X]=0; vec[Y]=0; vec[Z]=0; }
}

/**********************************************************************************************/

void glVector3fTriangleNormal
(GLfloat res[3], GLfloat v1[3], GLfloat v2[3], GLfloat v3[3])
{
  GLfloat       tv1[3], tv2[3];
  GLfloat       len;
...
void glVector3dTriangleNormal
(GLdouble res[3], GLdouble v1[3], GLdouble v2[3], GLdouble v3[3])
{
...
{
  if (vec[W]==0.0) vec[W]=1.0/0.00001; else vec[W]=1.0/vec[W];
  vec[X]*=vec[W];
  vec[Y]*=vec[W];
  vec[Z]*=vec[W];
}
...
{
  register GLU32 i, j;
  for (i=0 ; i<4 ; i++) for (j=0 ; j<4 ; j++) mat[i][j]=0.0;
}
...
void glMatrix4fTimes (GLmatrix4f(res), GLmatrix4f(mat1), GLmatrix4f(mat2))
{
  ...
  for (i=0; i<4; i++) for (j=0; j<4; j++) res[i][j]=temp[i][j];
}
...
void glMatrix4dConcat
(GLdouble res[4][4], GLdouble mat1[4][4], GLdouble mat2[4][4], GLenum op)
{
  switch (op)
  {
...
void glMatrix4fTranspose (GLmatrix4f(res), GLmatrix4f(mat))
{
  register GLU32        i, j;
  GLmatrix4f            (temp);
  for (i=0; i<4; i++) { for (j=0; j<4; j++) { temp[i][j]=mat[j][i]; } }
  for (i=0; i<4; i++) { for (j=0; j<4; j++) { res[i][j]=temp[i][j]; } }
}
...
void glMatrix4fTransVector3f (GLfloat* res, GLfloat* vec, GLmatrix4f(mat))
{
  ...
  tmp[X]=mat[0][0]*vec[X]+mat[0][1]*vec[Y]+mat[0][2]*vec[Z]+mat[0][3];
  tmp[Y]=mat[1][0]*vec[X]+mat[1][1]*vec[Y]+mat[1][2]*vec[Z]+mat[1][3];
  tmp[Z]=mat[2][0]*vec[X]+mat[2][1]*vec[Y]+mat[2][2]*vec[Z]+mat[2][3];
...
void glMatrix4dTransVector4d (GLdouble* res, GLdouble* vec, GLmatrix4d(mat))
{
  ...
  tmp[X]=mat[0][0]*vec[X]+mat[0][1]*vec[Y]+mat[0][2]*vec[Z]+mat[0][3]*vec[W];
  tmp[Y]=mat[1][0]*vec[X]+mat[1][1]*vec[Y]+mat[1][2]*vec[Z]+mat[1][3]*vec[W];
  tmp[Z]=mat[2][0]*vec[X]+mat[2][1]*vec[Y]+mat[2][2]*vec[Z]+mat[2][3]*vec[W];
  tmp[W]=mat[3][0]*vec[X]+mat[3][1]*vec[Y]+mat[3][2]*vec[Z]+mat[3][3]*vec[W];
...
void TV3M4 (GLdouble* res, GLdouble* vec, GLdouble mat [4][4])
{
  ...
  if (tmp[W]==0.0) tmp[W]=1.0/0.00001; else tmp[W]=1.0/tmp[W];
...
void glMatrix4dRotationLine
(GLdouble res[4][4], GLdouble angle, GLdouble p1[3], GLdouble p2[3])
{
  GLdouble      tmp[4][4];
  GLdouble      v1[3];
  GLdouble      v2[3];
  ...
  if (v1[X]==0.0 && v1[Y]==0.0 && v1[Z]==0.0) return;
